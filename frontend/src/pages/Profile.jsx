...
import getUserCampaigns from '../store/getUserCampaigns';
...
const Profile = () => {
  ...
  const fetchCampaigns = async () => {
    ...
    try {
      ...
      const data = await getUserCampaigns(); // data is an array of specific user campaigns objects.
      // if (!data) return []; The function getUserCampaigns() already returns an empty array in case of an error. So, checking if (!data) return []; is unnecessary.
      setCampaigns(data);

    } catch (error) {
      console.error("Error fetching campaigns:", error);
      // return []; The function getUserCampaigns() already returns an empty array in case of an error.
    }
